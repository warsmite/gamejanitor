...
const tokenColumns = "id, name, hashed_token, token_prefix, role, can_create, max_gameservers, max_memory_mb, max_cpu, max_storage_mb, claim_code, created_at, last_used_at, expires_at"
...
func scanToken(scan func(dest ...any) error) (model.Token, error) {
	var t model.Token
	err := scan(&t.ID, &t.Name, &t.HashedToken, &t.TokenPrefix, &t.Role, &t.CanCreateGS, &t.MaxGameservers, &t.MaxMemoryMB, &t.MaxCPU, &t.MaxStorageMB, &t.ClaimCode, &t.CreatedAt, &t.LastUsedAt, &t.ExpiresAt)
	return t, err
}
...
func (s *TokenStore) CreateToken(t *model.Token) error {
	...
	_, err := s.db.Exec(
		"INSERT INTO tokens (id, name, hashed_token, token_prefix, role, can_create, max_gameservers, max_memory_mb, max_cpu, max_storage_mb, claim_code, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
		t.ID, t.Name, t.HashedToken, t.TokenPrefix, t.Role, t.CanCreateGS, t.MaxGameservers, t.MaxMemoryMB, t.MaxCPU, t.MaxStorageMB, t.ClaimCode, t.CreatedAt, t.ExpiresAt,
	)
	if err != nil {

package gameserver

import (
	"context"
	"log/slog"
	"sync"

	"github.com/warsmite/gamejanitor/controller/event"
	"github.com/warsmite/gamejanitor/controller/cluster"
)

// OperationClearer clears the active operation for a gameserver.
// Implemented by operation.Tracker.
type OperationClearer interface {
	ClearOperation(gameserverID string)
}

// StatusSubscriber listens to lifecycle events on the bus and derives gameserver
// status from them. This centralizes status logic in one place instead of 25+
// scattered setGameserverStatus calls.
type StatusSubscriber struct {
	store       Store
	log         *slog.Logger
	bus         *event.EventBus
	querySvc    *cluster.QueryService
	statsPoller *cluster.StatsPoller
	operations  OperationClearer
	cancel      context.CancelFunc
	wg          sync.WaitGroup
}

func (s *StatusSubscriber) SetOperationClearer(oc OperationClearer) {
	s.operations = oc
}

func NewStatusSubscriber(store Store, bus *event.EventBus, querySvc *cluster.QueryService, statsPoller *cluster.StatsPoller, log *slog.Logger) *StatusSubscriber {
	return &StatusSubscriber{
		store:       store,
		bus:         bus,
		querySvc:    querySvc,
		statsPoller: statsPoller,
		log:         log,
	}
}

func (s *StatusSubscriber) Start(ctx context.Context) {
	ctx, s.cancel = context.WithCancel(ctx)

	ch, unsub := s.bus.Subscribe()
	s.wg.Add(1)
	go func() {
		defer s.wg.Done()
		defer unsub()
		for {
			select {
			case <-ctx.Done():
				return
			case evt, ok := <-ch:
				if !ok {
					return
				}
				s.handleEvent(evt)
			}
		}
	}()

	s.log.Info("status subscriber started")
}

func (s *StatusSubscriber) Stop() {
	if s.cancel != nil {
		s.cancel()
	}
	s.wg.Wait()
	s.log.Info("status subscriber stopped")
}

func (s *StatusSubscriber) handleEvent(evt event.WebhookEvent) {
	// Status is derived on read. Polling is managed by StatusManager.
	// The subscriber only clears operations on terminal events.
	e, ok := evt.(event.Event)
	if !ok {
		return
	}
	switch e.Type {
	case event.EventGameserverReady, event.EventInstanceStopped, event.EventInstanceExited, event.EventGameserverError:
		s.clearOperation(e.GameserverID)
	}
}

func (s *StatusSubscriber) clearOperation(gameserverID string) {
	if s.operations != nil {
		s.operations.ClearOperation(gameserverID)
	}
}

